...
class SBGlobalExtractor(BaseFeaturesExtractor):
    ...
    def __init__(
        ...
        # Shapes
        C_in, H, W = typed_obs_space["superblock"].shape
    ...
    def forward(self, obs: ObservationDict) -> tuple[th.Tensor, th.Tensor]:
        sb_feat: th.Tensor = self.conv_sb(obs["superblock"])
        frame_feat: th.Tensor = self.mlp_glob(obs["frame"])
