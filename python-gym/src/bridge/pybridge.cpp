                                       ...
                                       int32_t picture_number, int32_t frame_type, void *user)
{
    ...
    py::object ret = fcn.operator()(sb_info_list, sb_count, picture_number, frame_type);

    if (!py::isinstance<py::list>(ret)) {
        throw py::type_error(
            "Expected return value of type list however was " + py::cast<std::string>(ret.get_type().attr("__name__"))
        );
    }
    ...
    if (qp_map.size() != sb_count) {
        throw py::value_error(
            "Expected return value of type list with size " + std::to_string(sb_count) +" however was of size " + std::to_string(qp_map.size())
        );
    }
    ...
    for (uint32_t i = 0; i < sb_count; ++i) {
        ...
        if (!py::isinstance<py::int_>(item)) {
            throw py::type_error(
                "qp_map[" + std::to_string(i) + "] is not an int, got " +
